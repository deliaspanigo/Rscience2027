...
function initTree() {
  ...
  // ESTADO INICIAL: Raíz naranja y expandida
  activeNode = root;
  update(root);
}
...
function update(source) {
  ...
  nodeUpdate.select('circle')
      .style('fill', d => (d === activeNode || isAncestorOfActive(d)) ? '#ff9100' : '#00FFFF')
  ...
  linkEnter.merge(link).transition().duration(duration)
      ...
      .style('opacity', d => (d === activeNode || isAncestorOfActive(d)) ? 1 : 0.4);
  ...
  const width = container.node ? container.getBoundingClientRect().width : window.innerWidth;
  const height = container.node ? container.getBoundingClientRect().height : window.innerHeight;
...
// NUEVA FUNCIÓN DE VERIFICACIÓN DE ANCESTROS
...
ui <- fluidPage(
  tags$head(
    ...
    tags$style(HTML("
      ...
      .icon-snap { color: #ff9100; border-color: #ff9100; }
      .icon-snap:hover { background: #ff9100; color: #fff; box-shadow: 0 0 10px #ff9100; }
    ")),
  ...
  div(id = "tree-container",
      div(class = "internal-controls",
          ...
          div(id = "menu-content", class = "btn-stack",
              ...
              tags$button("🎯 Foco Selección", class = "btn-custom", onclick = "showSelectedOnly()"),
              tags$button("📷 Capturar Pantalla", class = "btn-custom icon-snap", onclick = "alert('Captura realizada')")
          )
      )
  ),
