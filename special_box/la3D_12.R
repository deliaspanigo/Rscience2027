...
var ghostMode = false, autoCollapseMode = false, topAlignMode = false;
...
function toggleAutoCollapse() { autoCollapseMode = !autoCollapseMode; if(autoCollapseMode) runCollapseOthers(); update(activeNode); }
...
function reorderPathToTop(d) {
  ...
  while (curr && curr.parent) {
    ...
    if (children) {
      ...
      if (idx > -1) { children.splice(idx, 1); children.unshift(curr); }
    }
    curr = p;
  }
}
...
function update(source) {
  if (autoCollapseMode) runCollapseOthers();
...
ui <- fluidPage(
  ...
  div(id = "tree-container",
      ...
      div(class = "panel-ctrl panel-right",
          ...
          div(class = "switch-container",
              span(class = "switch-label", "🎯 COLAPSAR OTROS"),
              tags$label(class = "switch", tags$input(type = "checkbox", onclick = "toggleAutoCollapse()"), span(class = "slider"))
          ),
          div(class = "switch-container",
