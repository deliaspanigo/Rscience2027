# ==============================================================================
# RSCIENCE 2026 - MODULAR TREE ENGINE (ROOT-FIX & PATH TRACKER) - v.0.0.1
# ==============================================================================
library(shiny)
library(bslib)
library(shinyjs)
...
tree_data <- tryCatch({
  ...
  data.frame(
    nivel1="Error", nivel2="Archivo", nivel3="No", nivel4="Encontrado", nivel5="Excel",
    size=10, script_id="ERR000"
  )
})
...
treeAppUI <- function(id) {
  ...
  tagList(
    ...
    tags$head(tags$style(HTML(paste0("
      ...
      /* === MOTOR DE COLOR: PRIORIDAD ABSOLUTA === */
      .nodo-ruta-activa circle {
    ...
    # === JAVASCRIPT ENGINE: PATH TRACKER FIXED ===
    tags$script(HTML(paste0("
      (function() {
        var activePathNames = ['START']; // Incluimos START por defecto
        var treeId = '", ns("stat_tree"), "';

        function highlightPath() {
          // 1. Iluminar Nodos (Círculos)
          d3.select('#' + treeId).selectAll('g.node')
            .classed('nodo-ruta-activa', function(d) {
              return activePathNames.includes(d.data.name);
            });

          // 2. Iluminar Enlaces (Líneas)
          d3.select('#' + treeId).selectAll('path.link')
            .classed('link-ruta-activa', function(d) {
              return activePathNames.includes(d.source.data.name) &&
                     activePathNames.includes(d.target.data.name);
            });
        }
        ...
        $(document).on('shiny:visualchange', function(event) {
          if (event.target.id === treeId) {
            ...
            // Forzar pintado inicial
            setTimeout(highlightPath, 100);

            // Evento Clic Directo en D3
            d3.select('#' + treeId).selectAll('circle.node').on('click.rscience', function(d) {
              activePathNames = [];
              var current = d;
              while (current) {
                activePathNames.push(current.data.name);
                current = current.parent;
              }
              highlightPath();
            });

            // MutationObserver para persistencia al interactuar
...
treeAppServer <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$selection_header <- renderUI({
      if (!is_confirmed()) {
        div(class = "selection-header",
            span(icon("hourglass-start"), " Navegando por el árbol estadístico..."),
            span(class = "header-id", paste("ID ACTUAL:", current_script_id()))
        )
      } else {
        path <- rev(input$node_click)
        div(class = "selection-header active",
            span(icon("check-circle"), paste(" CONFIRMADO:", paste(path, collapse = " > "))),
    ...
    output$tree_selection_ui <- renderUI({
      path <- input$node_click
      if (is.null(path) || length(path) == 0) return(p("Seleccione un nodo para comenzar..."))
      path_ordered <- rev(path)
      lapply(seq_along(path_ordered), function(i) {
        div(class = "path-chip", span(style="color:#aaa; font-size:0.7rem; margin-right:10px;", paste0("NIVEL ", i)), path_ordered[i])
      })
    })

    output$debug_ui <- renderUI({
      path <- input$node_click
      if (is.null(path) || length(path) == 0) return(div(class="debug-box", "SISTEMA STANDBY..."))
      div(class="debug-box",
          div(">> NODO SELECCIONADO: ", rev(path)[length(path)]),
          div(style="color:#fff; font-weight:bold; margin-top:10px;", ">> ASIGNACIÓN SCRIPT:"),
...
ui <- fluidPage(
  ...
  div(class = "container-fluid", style = "padding: 30px;",
      ...
      p("Rscience 2026 - v.0.0.1", style="color:#94a3b8; margin-bottom:30px;"),
      treeAppUI("main_tree")
  )
)
