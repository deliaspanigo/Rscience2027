...
# --- 2. JAVASCRIPT CON ZOOM PROACTIVO ---
js_logic <- "
...
function initTree() {
  ...
  update(root, true); // true indica que es el primer ajuste
}
...
function fitTreeInstant() {
  const container = d3.select('#tree-container').node();
  const width = container.getBoundingClientRect().width;
  const height = container.getBoundingClientRect().height;

  // Obtenemos los límites actuales del grupo G
  ...
  // La cámara se mueve AL MISMO TIEMPO que las ramas (mismo duration)
  d3.select('#tree-container svg')
    .transition().duration(duration).ease(d3.easeCubicOut)
    .call(zoom.transform, transform);
}

function update(source, isInit) {
  ...
  const nodeEnter = node.enter().append('g')
      ...
      .on('click', (event, d) => {
         ...
         // EJECUCIÓN PROACTIVA: Actualizamos datos y lanzamos zoom de inmediato
         update(d, false);
      });
  ...
  // LANZAMIENTO INMEDIATO DEL ZOOM
  // Usamos requestAnimationFrame para asegurar que el BBox tenga los nuevos nodos antes de mover la cámara
...
ui <- fluidPage(
  ...
  div(class = "controls",
      fluidRow(
        column(5, h2("Rscience INSTANT-FIT")),
