...
ui <- bslib::page_fluid(
  ...
  tagList(
      mod_03_A_theory_ui("test"),
      mod_03_A_theory_DEBUG_ui("test"),
  )
)
...
server <- function(input, output, session) {
  ...
  modo_activo <- mod_03_A_theory_server("test", folder_path_tool_script = the_folder_absolute, show_debug = T)
  ...
  observe({
    ...
    # Como el submódulo devuelve 'theory_meta' (otro reactive),
