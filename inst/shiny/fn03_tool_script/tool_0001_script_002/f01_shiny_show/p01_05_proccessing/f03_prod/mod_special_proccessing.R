...
mod_special_proccessing_server <- function(id, local_folder_tool_script, temp_folder_tool_script, list_quarto_replacement, show_debug = FALSE) {
  moduleServer(id, function(input, output, session) {
    ...
    render_file_row_server <- function(label, s) {
      icon_name <- switch(s, "pending" = "hourglass", "processing" = "spinner", "done" = "check-double", "error" = "times-circle", "hourglass")
      icon_class <- if(s == "processing") "fa-spin" else NULL
      color <- switch(s, "pending" = "#566b7a", "processing" = "#00d4ff", "done" = "#00bc8c", "error" = "#ff4b5c", "#566b7a")

      div(style = paste0("display: flex; align-items: center; padding: 8px 12px; background: #0b1218; ",
                         "border-left: 3px solid ", color, "; border-radius: 4px; margin-bottom: 6px;"),
          div(style = paste0("width: 8px; height: 8px; border-radius: 50%; margin-right: 12px; background:", color, "; box-shadow: 0 0 8px ", color)),
    ...
    output$item05_quarto_exec <- renderUI({
      ...
      total <- length(meta)
      completed <- if(length(current_status) > 0) sum(unlist(current_status) == "done", na.rm = TRUE) else 0
      ...
      div(style = "padding: 12px; background: #1a262f; border: 1px solid #2a3b47; border-radius: 8px;",
          div(style = "margin-bottom: 15px; font-weight: 800; font-size: 0.75rem; color: #adb5bd;", icon("microchip"), " ENGINE PIPELINE STATUS"),
    ...
    output$item01_folder_target <- renderUI({
      ...
      div(style = "margin-bottom: 10px; padding: 8px; background: #0b1218; border-radius: 4px;",
          icon("folder", style = "color: #00d4ff;"), span(" Local: ", style = "color: #adb5bd; font-size: 0.7rem;"),
    ...
    output$item02_folder_quarto_render <- renderUI({
      req(inputs_bundle())
      div(style = "margin-bottom: 10px; padding: 8px; background: #0b1218; border-radius: 4px;",
          icon("folder-open", style = "color: #00d4ff;"), span(" Temp: ", style = "color: #adb5bd; font-size: 0.7rem;"),
    ...
    output$item03_qmd_files <- renderUI({
      ...
      req(meta)
      div(style = "margin-bottom: 10px; padding: 8px; background: #0b1218; border-radius: 4px;",
          icon("file-alt", style = "color: #00d4ff;"), span(" QMD files: ", style = "color: #adb5bd; font-size: 0.7rem;"),
    ...
    output$item04_temp_folder_Rscience <- renderUI({
      div(style = "margin-bottom: 10px; padding: 8px; background: #0b1218; border-radius: 4px;",
          icon("info-circle", style = "color: #00d4ff;"), span(" Status: ", style = "color: #adb5bd; font-size: 0.7rem;"),
    ...
    output$debug_internal_ui <- renderUI({
      ...
      div(style = "margin-top: 20px; padding: 10px; background: #0b1218; border-radius: 4px; font-family: monospace; font-size: 0.7rem;",
          pre(paste("Engine started:", state$engine_started,
