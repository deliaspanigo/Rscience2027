...
mod_special_bibliography_DEBUG_ui <-  function(id) {
...
mod_special_bibliography_ui <- function(id) {
  ...
  tagList(
    ...
    navset_card_tab(
      full_screen = TRUE,
      title = span(icon("graduation-cap"), "RScience bibliography Engine"),
...
mod_special_bibliography_server <- function(id, show_debug) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns

    # 1. METADATOS: Corregido nombres de variables
    bibliography_meta <- reactive({
      ...
      list(
        "description" =  "Submodulo especial Bibliography",
        "help" =         "Check tool!!!",
        file_name       = file_name,
        full_path       = full_path,
        exists          = file.exists(full_path), # Antes era file_exists
        dir_path        = dirname(full_path),
        resource_prefix = paste0("bibliography_core_", id), # Faltaba en la lista
        timestamp       = Sys.time(),
    ...
    output$debug_internal <- listviewer::renderJsonedit({
      listviewer::jsonedit(bibliography_meta(), mode = "text")
    })
    ...
    output$show_debug_external <- renderUI({
      div(style = "background: #1a1a1a; padding: 15px; border-radius: 8px;",
          h5("DEBUG - Special bibliography", style="color: #00d4ff;"),
          listviewer::jsoneditOutput(ns("debug_external"), height = "500px")
      )
    })

    return(bibliography_meta)
  })
}
