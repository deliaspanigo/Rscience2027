...
mod_special_script_and_comments_ui <- function(id) {
  ns <- NS(id)
...
mod_special_script_and_comments_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    data_config <- reactive({
      ...
      target_dir <- file.path(base_path, "f02_quarto_proc", "f02_quarto_mod")
