...
mod_pipeline_server <- function(id, folder_target, list_settings) {
  moduleServer(id, function(input, output, session) {
    ...
    # Índice reactivo para el pack actual
    current_idx <- reactiveVal(0)

    # El "latido" del motor: cuando este cambia, se ejecuta un render.
    # Usamos debounce aquí para dar el respiro de 1s entre cada archivo.
    engine_heartbeat <- reactive({
      current_idx()
    }) %>% debounce(1000)

    observe({
      # 1. Requisitos de inicio
      req(engine$started, rlist_item06_qmd_files_temp()$is_done)

      # 2. Obtener la lista de archivos dinámicamente
      details <- rlist_item06_qmd_files_temp()$details
      pkg_names <- names(details)
      idx <- engine_heartbeat() # Escuchamos al corazón estabilizado

      # 3. Caso inicial: Si el botón se pulsó pero idx es 0, empezamos
      if(idx == 0) {
        current_idx(1)
        return()
      }

      # 4. Caso final: Si ya procesamos todos, nos detenemos
      if(idx > length(pkg_names)) {
        message("--- PIPELINE FINALIZADO ---")
        return()
      }

      # 5. EJECUCIÓN DEL PASO ACTUAL
      pkg_name <- pkg_names[idx]

      # Usamos isolate para que los cambios en render_status no reinicien este observe
      isolate({
        # A. Actualizar UI a "processing"
        render_status[[pkg_name]] <- "processing"

        # B. Renderizado Quarto
        tryCatch({
          quarto::quarto_render(
            input = details[[pkg_name]]$qmd_file_path_abs_temp,
            quiet = TRUE
          )
        ...
        # C. DISPARAR EL SIGUIENTE (Recursión reactiva)
        # Al cambiar este valor, 'engine_heartbeat' esperará 1000ms
        # y luego este 'observe' se ejecutará de nuevo para el siguiente idx.
        current_idx(idx + 1)
      })
    })
