...
mod_03_B_bibliography_DEBUG_ui <- function(id) {
  ns <- NS(id)
  # Este uiOutput debe coincidir con el ID del server
  uiOutput(ns("panel_debug_externo"))
}

mod_03_B_bibliography_ui <- function(id) {
  ns <- NS(id)

  tagList(
    navset_hidden(
      id = ns("bibliography_switcher"),
...
# Colócala fuera del mod_03_B_bibliography_server
ui_debug_layout_bibliography <- function(ns, prefix = "") {
...
mod_03_B_bibliography_server <- function(id, folder_path_tool_script, show_debug = TRUE) {
  moduleServer(id, function(input, output, session) {
    ...
    internal_meta <- reactive({
      ...
      target <- file.path(p, "f01_shiny_show", "p01_02_bibliography", "f03_prod", "mod_special_bibliography.R")
      list(
    ...
    observeEvent(internal_meta(), {
      ...
      if (info$status == "PATH_PROVIDED" && isTRUE(info$exists)) {
        ...
        tryCatch({
          ...
          if (!is.null(new_env$mod_special_bibliography_server)) {
            rv$sub_data <- new_env$mod_special_bibliography_server(id = "sub_bibliography")
          }
          rv$ready <- TRUE
      ...
      tagList(
        ...
        div(style = "padding: 80px 20px; text-align: center; border: 2px dashed #444; border-radius: 20px; background: #1a1a1a; transition: all 0.5s ease; overflow: hidden;",
            ...
            p("Complete the selection in the",
              ...
              tags$b("bibliography", style="color: #ffffff;"),
              "content.",
    ...
    output$placeholder_dinamico <- renderUI({
      req(rv$ready)
      env <- local_env()
      req(env$mod_special_bibliography_ui)
      env$mod_special_bibliography_ui(ns("sub_bibliography"))
    })
    ...
    observe({
      ...
      if (info$status == "WAITING_PATH") {
        nav_select("bibliography_switcher", "state_waiting")
      } else if (!isTRUE(rv$ready)) {
        nav_select("bibliography_switcher", "state_loading")
      } else {
        nav_select("bibliography_switcher", "state_ready")
      }
    })
    ...
    output$debug_internal <- renderUI({
      req(show_debug)
      ui_debug_layout_bibliography(ns, prefix = "")
    })
    ...
    output$panel_debug_externo <- renderUI({
      ...
      div(style = "border: 2px solid #00d4ff; padding: 10px; border-radius: 10px;",
          ...
          ui_debug_layout_bibliography(ns, prefix = "ext_")
      )
    })
