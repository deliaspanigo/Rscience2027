...
mod_03_B_bibliography_server <- function(id, folder_path_tool_script) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      target_file <- file.path(base_p, "f03_soft_opts", "f02_bibliography", "f03_prod", "mod_special_bibliography.R")
      ...
      if (file.exists(target_file)) {
        ...
        tryCatch({
          ...
          if (!is.null(new_env$mod_special_bibliography_server)) {
            new_env$mod_special_bibliography_server("sub_bi")
            message("--- [Collector] Server 'sub_bi' inicializado.")
          }
    ...
    output$placeholder <- renderUI({
      env <- local_env()
      req(env$mod_special_bibliography_ui) # Solo renderiza si la función existe en el entorno

      tagList(
        env$mod_special_bibliography_ui(ns("sub_bi"))
      )
    })
