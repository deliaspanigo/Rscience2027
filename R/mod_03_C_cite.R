...
mod_03_C_cite_server <- function(id, folder_path_tool_script) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      target_file <- file.path(base_p, "f03_soft_opts", "f03_cite", "f03_prod", "mod_special_cite.R")
      ...
      if (file.exists(target_file)) {
        ...
        tryCatch({
          ...
          if (!is.null(new_env$mod_special_cite_server)) {
            new_env$mod_special_cite_server("sub_cite")
            message("--- [Collector] Server 'sub_cite' inicializado.")
          }
    ...
    output$placeholder <- renderUI({
      env <- local_env()
      req(env$mod_special_cite_ui) # Solo renderiza si la función existe en el entorno
