...
mod_11_D_pdf_DEBUG_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
mod_11_D_pdf_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    internal_path <- reactive({
      if (is.function(module_pdf_file_path)) module_pdf_file_path() else module_pdf_file_path
    })
    ...
    observe({
      req(rv$ready)
      env <- local_env()
      req(!is.null(env$mod_special_pdf_server))
    ...
    output$placeholder_dinamico <- renderUI({
      req(rv$ready)
      env <- local_env()
      req(env$mod_special_pdf_ui)
      env$mod_special_pdf_ui(ns("sub_proc"))
    })
